function makeUpdateDocument({updateDocument, Joi,ValidationError,ForbiddenError }) {
  
    return async function updateDocumentFun({name,numShards,configSet }) {
        
        await validateInputData({name,numShards,configSet});
       return await updateDocument({name,numShards,configSet}); 
    };
