Feature: update Session By Id.

  Scenario Outline: Try to update session with invalid details, then it will throw error.
    Given Session details sessionId:"<sessionId>" to update session
    When Try to update session
    Then It will throw error:"<error>" with message:"<message>" while updating session
...
Scenario Outline: Try to update session with valid inputs, then it will success.
    Given Session details sessionId:'<sessionId>' to update session
    When  Try to update session
    Then  update session successfully message:"<message>"

    Examples:
      |     sessionId | message                     |
      |      fa1507c0-05f6-11ee-aacd-d5f50a890398        | update session successfully | 